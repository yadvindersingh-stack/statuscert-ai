...
export default function LoginPage({
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center px-6 py-10">
      <div className="w-full max-w-[900px] rounded-3xl border border-[var(--border)] bg-white shadow-[0_20px_48px_rgba(15,23,42,0.08)]">
        <div className="grid md:grid-cols-[0.95fr_1.05fr]">
          <div className="hidden rounded-l-3xl bg-[linear-gradient(160deg,#0f2742_0%,#173a60_100%)] p-10 text-white md:flex md:flex-col md:justify-between">
            <div>
              ...
              <h2 className="mt-3 font-serif text-3xl leading-tight">Condo status certificate drafting for Ontario legal teams.</h2>
            </div>
            <p className="text-sm text-white/80">Upload package. Generate draft. Edit and export DOCX.</p>
          </div>
          <div className="p-8 md:p-10">
            <h1 className="font-serif text-3xl font-semibold">Log in</h1>
            <p className="mt-2 text-sm text-slate">Access your firm workspace.</p>
        {searchParams?.error ? (
        ...
        {searchParams?.message ? (
          ...
          <div>
            <label className="text-xs uppercase tracking-[0.2em] text-slate">Password</label>
              ...
              <FormSubmitButton className="btn btn-primary mt-2 w-full disabled:opacity-60" idleLabel="Log in" pendingLabel="Logging in..." />
        </form>
            <p className="mt-5 text-sm text-slate">
              New to StatusCert AI? <Link className="text-ink underline" href="/signup">Create your firm account</Link>
        </p>
          </div>
        </div>
      </div>
