...
import { CANONICAL_REVIEW_SECTIONS, canonicalizeReviewSections, reviewTextToSections, sectionsToReviewText } from './editor';
...
function formatUnitForTitle(value?: string | null) {
  if (!value) return '';
...
function removeDuplicateUnitFromAddress(address: string, unit?: string | null) {
  if (!address) return address;
  const unitToken = normalizeUnitToken(unit);
  if (!unitToken) return address;
...
function shouldPrefixUnit(cleanUnit: string, cleanAddress: string) {
  if (!cleanUnit || !cleanAddress) return false;
...
function buildAutoReviewTitle(extracted: ExtractedJson) {
  ...
  const subject =
    (cleanUnit && cleanAddress && shouldPrefixUnit(cleanUnit, cleanAddress) && `${cleanUnit} - ${cleanAddress}`) ||
    cleanAddress ||
    cleanUnit ||
...
function buildReviewTitleFromExtraction(extracted: ExtractedJson) {
...
function buildReviewFilenameStem(title: string, reviewId: string) {
...
function hasPresentValue(value?: string | null) {
  if (value === null || value === undefined) return false;
  const normalized = String(value).trim();
  return normalized.length > 0;
}

function isNegativeOrNone(value?: string | null) {
  if (!hasPresentValue(value)) return false;
  const normalized = String(value).toLowerCase();
  return /\bnone\b|\bno\b|\bnot found\b|\bnil\b|\bnot disclosed\b|\bnot noted\b/.test(normalized);
}

function shortValue(value?: string | null, max = 140) {
...
function buildConciseSummaryBullets(extracted: ExtractedJson) {
  const bullets: string[] = [];

  bullets.push(
    hasPresentValue(extracted.leased_unit_count)
  ...
  bullets.push(
    hasPresentValue(extracted.special_assessments) && !isNegativeOrNone(extracted.special_assessments)
  ...
  bullets.push(
    hasPresentValue(extracted.reserve_fund_balance)
  ...
  bullets.push(
    hasPresentValue(extracted.legal_proceedings) && !isNegativeOrNone(extracted.legal_proceedings)
...
function isFlagAllowedForLawyerSummary(flag: FlagItem) {
  const key = String(flag.key || '').toLowerCase();
  const title = String(flag.title || '').toLowerCase();
  if (key.startsWith('missing_')) return false;
  if (title.includes('parking') || title.includes('locker') || title.includes('bike')) return false;
  if (title.includes('no mention') || title.includes('not found') || title.includes('no explicit')) return false;
  return true;
}

function buildConciseFollowUpSection(extracted: ExtractedJson, flags: FlagItem[]) {
...
function dedupeFlags(flags: FlagItem[]) {
  const seen = new Set<string>();
...
function isActionableUnusualClause(clause: string) {
...
function sanitizeUnusualClauses(clauses: string[]) {
...
function reconcileExtractedFacts(rawText: string, extracted: ExtractedJson) {
  ...
  const clearIfFound = (key: keyof ExtractedJson) => {
  ...
  [
    ...
    'special_assessments',
    'restrictions_summary'
  ].forEach((key) => clearIfFound(key as keyof ExtractedJson));
...
function detectCriticalClauses(rawText: string) {
  const text = rawText || '';
...
function enforceDeterministicQuality(
  ...
  mergedText: string
): { sections: ReviewSection[]; flags: FlagItem[]; followUps: string[]; qa: Record<string, unknown> } {
  const outFlags: FlagItem[] = [];
  const outFollowUps: string[] = [];
  ...
  const mandatoryInsuranceLinePresent = nextSections.some(
    (s) => s.key === 'insurance' && /secured all policies of insurance required under the condominium act, 1998/i.test(s.content || '')
  );
  ...
  const mustHaveEvidence = [...MUST_HAVE_FIELDS].reduce(
    (acc, field) => {
      const evidence = findEvidenceByField(extracted, [field]);
      if (evidence.length) acc.withEvidence += 1;
      return acc;
    },
    { withEvidence: 0, total: MUST_HAVE_FIELDS.size }
  );

  let status: 'PASS' | 'PASS_WITH_GAPS' | 'FAIL' = 'PASS';
...
export async function runGenerateDraftJob(job: any) {
  ...
  const normalizedMissingFields = missingFields.filter((fieldKey) => {
    const value = (reconciled.extracted as Record<string, unknown>)[fieldKey];
  ...
  const alwaysShowFlags: FlagItem[] = [];
  ...
  const finalSections = canonicalizeReviewSections([
    {
  ...
  delete (reconciled.extracted as ExtractedJson & { quality_checks?: Record<string, unknown> }).quality_checks;
...
export async function runExportDocxJob(job: any) {
  ...
  const canonicalTitle = getCanonicalReviewTitle(review.title, (review.extracted_json || null) as ExtractedJson | null);
  ...
  const sectionsForDocx =
    Array.isArray(review.review_sections_json) && review.review_sections_json.length
      ? canonicalizeReviewSections((review.review_sections_json || []) as ReviewSection[], review.review_text)
      : review.review_text && review.review_text.trim()
      ? canonicalizeReviewSections(reviewTextToSections(template.sections, review.review_text), review.review_text)
      : canonicalizeReviewSections([]);
