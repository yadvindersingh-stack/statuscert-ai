...
async function extractStatusCert(text) {
    ...
    if (!Array.isArray(extracted.unusual_clauses))
        extracted.unusual_clauses = [];
    if (!extracted.aps_extracted)
        extracted.aps_extracted = { aps_present: false };
    if (!Array.isArray(extracted.aps_extracted.evidence))
        extracted.aps_extracted.evidence = [];
