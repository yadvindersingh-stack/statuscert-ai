...
function normalizeDateText(value, fallback = "Not available") {
    const text = toText(value, fallback);
    if (!text || text === fallback)
        return text;
    return text.replace(/\b\d{4}-\d{2}-\d{2}\b/g, (token) => formatHumanDate(token));
}
function normalizeLegalProceedings(value) {
...
function toConciseLine(value, fallback = "Not available", max = 220) {
...
function pickFirstMatchingSentence(value, matchers, fallback = "Not available") {
...
function normalizeSpecialAssessmentCell(yesNo, detail) {
    if (yesNo !== "Yes")
        return yesNo;
    if (!detail || detail === "Not available" || detail === "Not found" || detail === "NONE")
        return "Yes";
    return "Yes";
}
...
async function buildFromMasterTemplate(input, templatePath) {
    ...
    const petSummary = toConciseLine(extracted.pet_summary || pickFirstMatchingSentence(restrictionCorpus, [/\bpet\b/, /\banimal\b/, /\bnuisance\b/], ""), "Not available", 220);
    const leasingSummary = toConciseLine(extracted.leasing_summary || pickFirstMatchingSentence(restrictionCorpus, [/\bleas/, /\btenant\b/, /\bsublet\b/, /\bshort[- ]?term\b/], ""), "Not available", 220);
    const permittedUse = toConciseLine(extracted.permitted_use_summary || pickFirstMatchingSentence(restrictionCorpus, [/\buse\b/, /\boccupan/, /\bsingle[- ]family\b/, /\bresidential\b/], ""), "Not available", 220);
    ...
    xml = replacePlaceholderAfterAnchor(xml, "Levied Special Assessments", "Yes or No", xmlEscape(specialAssessmentRenderedValue));
    ...
    if (unresolvedRequired.length || bannedResidueHits.length || missingRequiredAnchors.length) {
        throw new ExportTemplateMappingError(`Template mapping incomplete. unresolved_required=${unresolvedRequired.length}, residue=${bannedResidueHits.join(", ")}, anchors_missing=${missingRequiredAnchors.length}`, diagnostics);
    }
