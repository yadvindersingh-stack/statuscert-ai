  ...
  jsonb_set(
    jsonb_set(
      jsonb_set(
        template_json,
        '{mode}',
        '"precedent_locked"'::jsonb,
        true
      ),
      '{disclaimers}',
      '["Facts are drawn from the provided status certificate package and should be verified against the source documents.","This review does not replace independent legal analysis or partner review."]'::jsonb,
      true
    ),
    '{title}',
    '"Status Certificate Review – Precedent"'::jsonb,
    true
  ),
  '{sections}',
  '[
...
updated_at = now()
where firm_id is null and is_default = true;
